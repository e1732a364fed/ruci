...
impl SmoltcpDevice {
    ...
    pub fn new(
        ...
        Self,
        Receiver<(SocketHandle, SocketAddr, BytesMut)>,
        Receiver<(SocketHandle, IpEndpoint, BytesMut)>,
        Receiver<BytesMut>,
    ) {
        ...
        let d = Self {
            cid,
            traffic: Traffic::new(),
            buf: Box::new([0; u16::MAX as usize]),
            w,
            r,
    ...
    fn check_read_buf_for_new_conn(&mut self, n: usize) {
        ...
        let packet = super::ip_packet::IpPacket::new_checked(data)
            .context("convert frame to IpPacket failed");
