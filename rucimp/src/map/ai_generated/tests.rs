...
async fn test_basic_write_sequence() -> Result<()> {
    init_tracing();
    println!("test_basic_write_sequence start");
    // 启动模拟服务器
    let mock_server = MockServer::start().await;
    ...
    let server_handle = tokio::spawn(async move {
        ...
        tracing::debug!("server_handle read done");
        // 发送 r1 响应
        let r1_data = vec![1u8; 10];
        server_tcp.write_all(&r1_data).await?;

        tracing::debug!("server_handle write done");
    ...
    // let (mut r, mut w) = tokio::io::split(ai_conn);
    ...
    tracing::debug!("server_handle done");
