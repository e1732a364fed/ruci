...
mod test {
    ...
    fn test1() {
        ...
        let fs = crate::utils::FileSource::default();
        ...
        //www.baidu.com's IP
        let s = get_ip_iso("104.193.88.123".parse().unwrap(), "Country.mmdb", &fs);
        println!("{s}");
        assert_eq!(s, "CN");
        assert_eq!(filter_iso_string_to_iso3166(&s), "CN");

        // www.google.com's IP
        let s = get_ip_iso("142.251.32.36".parse().unwrap(), "Country.mmdb", &fs);
        println!("{s}");
        assert_eq!(s, "GOOGLE");
        assert_eq!(filter_iso_string_to_iso3166(&s), "US");

        // www.twitter.com's IP
        let s = get_ip_iso("104.244.42.1".parse().unwrap(), "Country.mmdb", &fs);
        println!("{s}");
        assert_eq!(s, "TWITTER");

        // www.reddit.com's IP
        let s = get_ip_iso("151.101.65.140".parse().unwrap(), "Country.mmdb", &fs);
        println!("{s}");
        assert_eq!(s, "FASTLY");
        assert_eq!(filter_iso_string_to_iso3166(&s), "US");
    }
