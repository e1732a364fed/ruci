...
fn get_g_map_from(
    ...
    anyhow::Context::context(lua.load(lua_text).exec(), "eval lua failed")?;

    let t: LuaTable = anyhow::Context::context(
        lua.globals().get(INFINITE_CONFIG_FIELD),
        "get Infinite Global field failed",
    )?;
    ...
    let len = {
        let t: LuaTable =
            anyhow::Context::context(t.get(t_key), format!("get '{}' field failed", t_key))?;
        t.len()?
    };
    // lua 的 index 是从 1 算起
    for i in 1..len + 1 {
        ...
        anyhow::Context::context(
            ...
            format!("eval lua for {}'s {} item failed", t_key, i),
        )?;
        ...
        let (key, tag) = {
            ...
            let tag: String = anyhow::Context::context(chain.get("tag"), "get 'tag' field failed")?;
            let g: LuaFunction = anyhow::Context::context(
                chain.get(GENERATOR_FIELD),
                "get 'generator' field failed",
            )?;
