...
pub trait DynIterator {
    fn next_with_data(&mut self, data: Option<Vec<Data>>) -> Option<Arc<MapperBox>>;
...
impl DynIterator for DynMIterWrapper {
    fn next_with_data(&mut self, _data: Option<Vec<Data>>) -> Option<Arc<MapperBox>> {
        self.0.next()
    }
...
pub async fn accumulate(params: AccumulateParams) -> AccumulateResult {
    ...
    let mut calculated_output_vec: Vec<Data> = Vec::new();
